            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex m-b-10 no-block">
                        <h5 class="card-title m-b-0 align-self-center">Location Management</h5>
                        <div class="ml-auto text-light-blue">
                            <ul class="nav nav-tabs customtab default-customtab list-inline text-uppercase lp-5 font-medium font-12" role="tablist">
                                <li>
                                    <a href="{{url('admin/location/create/')}}" class="btn waves-effect waves-light btn-rounded btn-primary">Add Location</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive m-t-10">
                            <table id="myTable" class="table color-table table-bordered product-table table-hover">
                                ...
                                @foreach($locations as $key => $loc)
                                    <tr>
                                        <td>{{ $key +1 }}</td>
                                        <td><?= date_format($loc->created_at,"d-M-Y") ?></td>
                                        <td>{{ $loc->location }}</td>
                                      <!--   @if($loc->status == 1)
                                        ...
                                        <td class="text-center">
                                            <a href="{{ url('/admin/location/'.$loc->id.'/edit/') }}"><i class="fas fa-edit"></i></a>
                                             <form style="display: inline-block;" method="POST" action="{{ url('/admin/location',$loc->id) }}">
                                                @csrf
                                                @method('DELETE')
