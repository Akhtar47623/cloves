                ...
                <div class="card-body">
                    <div class="d-flex m-b-10 no-block">
                        ...
                        <div class="ml-auto text-light-blue">
                            <ul class="nav nav-tabs customtab default-customtab list-inline text-uppercase lp-5 font-medium font-12" role="tablist">
                                <a href="{{url('admin/depot-location/create')}}" class="btn waves-effect waves-light btn-rounded btn-primary">Add Depot</a>
                            </ul>
                        </div>
                    </div>
                    @if($depotLoc)
                    <div class="table-responsive m-t-10">
                        <table id="myTable" class="table color-table table-bordered product-table table-hover">
                            ...
                            <tbody>
                                @foreach($depotLoc as $key=> $depot)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td>{{ $depot->depot_location }}</td>
                                    <td>{{ $depot->radius }} Miles</td>
                                     @if($depot->status == 1)
                                        <td><span class="badge-success p-2">Active</span></td>
                                        @elseif($depot->status == 0)
                                        <td><span class="badge-danger p-2">Inactive</span></td>
                                        @endif
                                    <td class="text-center">
                                        <a href="{{ url('admin/depot-location/'.$depot->id.'/edit/') }}"><i class="fas fa-edit"></i></a>

                                       <form style="display: inline-block;" method="POST" action="{{ url('/admin/depot-location',$depot->id) }}">
                                                @csrf
                                                @method('DELETE')
